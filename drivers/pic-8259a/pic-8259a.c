 ...
 * where:
 * \li \c LTIM stands for Level Triggered Interrupt Mode, hence is 1 for level triggered mode, 0 for
 * edge triggered;
 * \li \c SNGL stands for Single or cascade, hence 1 for single PIC mode, 0 for cascade PIC mode.
 * \li \c IC4 stands for Initialization Control Word 4 needed , hence 1 if we are going to send IC4,
 * 0 otherwise.
 * For standard PC/AT PIC we are going to have:
 * \l1  edge triggered interrupts;
 * \l1  cascade mode;
 * \l1  we do send IC4;
 * Given all that, the very first thing we send to both PICs is byte 00010001b = 0x11. Remembering
 * that A0 line must
 * be held low, we output 0x11 to ports (addresses) 0x20 for master PIC and 0xA0 for the slave PIC.
 * When a PIC recognizes ICW1, it expects at least ICW2 and ICW3 to follow. If indicated, as we
 * usually do, that ICW4 follows, then it must be send as well.
 * After initialization is complete, one may read or write Operation Control Words (OCWs for short)
 * in any order, until she or he sends ICW1 again.
 * Next initialization word to be send is ICW2, whose format is as follows (for 8086 ubiquitous
 * mode):
 * <tt>bit  | 7  | 6  | 5  | 4  | 3  | 2 | 1 | 0 |</tt>
 * <tt>value    | T7 | T6 | T5 | T4 | T3 | 0 | 0 | 0 |</tt>
 * where \c T7 .. \c T3 are the bits being placed on the data bus during interrupt acknowledge
 * cycle. The three least significant bits will be equal to the interrupt being issued.
 * Hence, if for instance, we send ICW2 as 0x20 to PIC1, then it will send 0x20 whenever interrput 1
 * is signaled, and 0x21 when interrupt 2 is signaled, and so on.
 * In protected mode, we usually configure PIC so that master ICW2 points to 0x20 and slave's ICW2
 * pointing right after it at 0x28. As the D4 bit may be sometimes set to 1, we
 * must send this ICW2 with A0 line held high. Therefore, we do send ICW2 to port 0x21 for master
 * PIC and to port 0xA1 to slave PIC.
 * \p
 * Then goes Initialization Control Word 3. This is actually used in in PC AT. This one is here to
 * tell each PIC about each other. Its meaning its different for the master and the slave.
 * For the master it is a bit mask that tells which input is hooked up to slave controller. For
 * slave it's the same, but it's encoding differs - its three least significant bits are the
 * numerical value of that
 * input. For IBM PC/AT machine, the IBM engineers decided for us. The hooked up slave PIC so that
 * it is connected to interrupt request line 2 of the slave PIC (interrupt request lines are 0..7).
 * It seems like we need to send 00000100b = 0x04 to the master and 0x02 to the slave. Again, to
 * avoid clash with ICW1, A0 line must be held high. Otherwise, a PIC logic may decide that this is
 * ICW1 and we would need
 * to start initialization all over again.
 * \p
 * Finally goes the Initialization Control Word 4, ICW4 for short. This is pretty important, because
 * it actually tells a PIC, among other things, in which mode it will work from now on.
 * If we don't send it, then it will assume that it's mode is MCS 8080/8085. In short, without ICW4
 * your PC, whether it is XT or AT, "will be in a world of shit" and your system will die without
 * permission.
 * Format of the ICW4 is as follows:
 * <tt>bit  | 7 | 6 | 5 | 4    | 3  | 2 | 1    | 0    |</tt>
 * <tt>value    | 0 | 0 | 0 | SNFM | BUFMS  | AEOI | miPM |</tt>
 * which stand for:
 * \l1 \c SNFM - Special Fully Nested Mode, hence 1 for Special Fully Nested Mode and 0 for not
 * Special Fully Nested Mode.
 * \l1 \c BUFMS - a pair of bits that indicate whether it is Non buffered mode (b0x = b0whatever =
 * 00 or 01), buffered mode master (10b) or buffered mode slave (11b)
 * \l1 \c AEOI - automatic End Of Interrupt, hence 1 for AEOI and 0 for standard EOI.
 * \l1 \c miPM - stands for microprocessor Programming Mode, hence 1 for 8086 and 0 for MCS
 * 8080/8085.
 * Usually, we only indicate 8086 mode in our ICW4 being send on PC. That means that we need to send
 * 00000001b = 0x01 to both PICs.
 ...
 * after all those chores are completed, we are done initializing our PIC. From now on, until ICW1
 * is submitted again, all reads and writes
 ...
 * @details PIC interrupt mask is a special Operation Control Word 1, a byte whose can be read or
 * written to PIC. PIC recognizes that a request is for OCW1 if the A0 line is held high.
 * Format of the OCW1 is pretty simple - it is a 8 bit mask, whose bits set to 1 at various
 * positions do indicate that an input does not accept interrupts.
 * @param
 * @retval
 */
 ...
 * @details The EOI is just a single bit in Operation Control Word 2. Other bits are unused in 8086
 * system. Until EOI is send,
 * all the interrupts of the same or lower priority will be inhibited.
 * @param irq_num - interrupt number, used to determine whether to send EOI to just master or to
 * both PICs.
 */
